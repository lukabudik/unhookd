...
function loadActualHistory(plan: TaperPlan): { day: number; total: number }[] {
  ...
  const planTotalDays = plan.daysToTarget !== undefined ? plan.daysToTarget : plan.weeksToTarget * 7
  const lookback = Math.min(daysSinceStart, planTotalDays)
...
export function TaperTrajectoryChart({ plan }: Props) {
  ...
  // Y-axis labels
  const yLabels = [plan.startAmount, plan.startAmount / 2, plan.targetAmount].filter(
    (v, i, a) => a.indexOf(v) === i
  )
  ...
  return (
    ...
    >
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible', display: 'block' }}>
        ...
        <polygon
          points={[`${PAD_L},${toY(0)}`, ...planPoints, `${toX(totalDays)},${toY(0)}`].join(' ')}
        ...
        {todayDay > 0 && (
          <>
            <line
              ...
              strokeDasharray="3 3"
            />
            <circle cx={todayX} cy={toY(todayTarget)} r="5" fill="var(--primary)" />
          </>
        )}
      </svg>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 4, padding: '0 4px 4px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <div
            style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--danger)' }}
          />
          <span style={{ fontSize: 11, color: 'var(--text-secondary)' }}>Over target</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <div
            style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--primary)' }}
          />
