...
function toLocalDate(utcNow, utcOffsetMinutes) {
...
function buildDailyReminder(plan, localNow, todayTotal, streak) {
    const target = getDailyTarget(plan, localNow);
    const dayNum = Math.floor((localNow.getTime() - new Date(plan.startDate).getTime()) / 86400000) + 1;
...
function buildStreakMilestone(streak, plan, localNow) {
    const dayNum = Math.floor((localNow.getTime() - new Date(plan.startDate).getTime()) / 86400000) + 1;
...
function buildWeeklySummary(daysLogged, daysOnTarget) {
    if (daysLogged === 0)
        return {
...
function buildMondayTarget(plan, localNow) {
    const target = getDailyTarget(plan, localNow);
...
function buildHoldEnding(plan, localNow) {
...
function buildReEngagement(daysSince) {
    if (daysSince >= 5)
        return {
...
exports.sendScheduledNotifications = (0, scheduler_1.onSchedule)({
...
async function processUser(utcNow, recoveryCode, fcmRef, fcmData) {
    ...
    const planSnap = await db
        .doc(`users/${recoveryCode}/data/plan`)
        .get();
    if (!planSnap.exists)
        return;
    ...
    if (todayTotal > 0 && todayTotal <= getDailyTarget(plan, localNow))
        streak++;
    ...
    if (fcmData.reminderTime && fcmData.dailyReminderDate !== localDate) {
        ...
        if (localHour === rh) {
            ...
            if (tokenValid)
                updates.dailyReminderDate = localDate;
        }
    }
    if (!tokenValid) {
        await fcmRef.delete();
    ...
    if (localHour === 20 && fcmData.eveningCheckinDate !== localDate && todayTotal === 0) {
        ...
        if (tokenValid)
            updates.eveningCheckinDate = localDate;
    }
    ...
    if (localDayOfWeek === 0 && localHour === 20 && fcmData.weeklySummaryKey !== wKey) {
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            if (total > 0) {
                daysLogged++;
                if (total <= getDailyTarget(plan, d))
                    daysOnTarget++;
            }
        }
    ...
    if (plan.holdUntil) {
        ...
        if (holdEnd === tomorrow && fcmData.holdEndingSentFor !== holdEnd) {
            ...
            if (tokenValid)
                updates.holdEndingSentFor = holdEnd;
        }
    }
    // ── 7. Streak milestones ──────────────────────────────────────────────
